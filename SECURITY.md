...
Article HTML is rendered via `dangerouslySetInnerHTML`. The current implementation relies on the source feeds being well-formed. For deployments where users subscribe to untrusted feeds, run extracted content through DOMPurify before rendering:

```typescript
import DOMPurify from 'isomorphic-dompurify';
const safe = DOMPurify.sanitize(article.contentHtml);
```

This is a TODO for a future hardening pass.
...
| Insecure Design | ⚠️ Article HTML rendered without DOMPurify (see above) |
...
The two `⚠️` items above are the main hardening priorities if you go beyond personal use.
